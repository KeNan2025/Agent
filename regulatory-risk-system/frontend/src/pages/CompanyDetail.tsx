        ...
        <Col xs={24} lg={16}>
          ...
          >
            {data.shap_features.map((f: any, i: number) => {
              const barColor = f.shap_value > 0.05 ? '#ef4444' : f.shap_value > 0.02 ? '#f59e0b' : '#1a5cff';
...
function FinancialPanel({ financial }: { financial: any }) {
  ...
  const groups = [
    {
      title: '盈利能力', icon: <RiseOutlined style={{ color: '#1a5cff' }} />,
      items: [
  ...
  const columns = [
    {
      ...
      render: (_: any, __: any, i: number) => (
        <Badge count={i + 1} style={{
          backgroundColor: i < 3 ? '#1a5cff' : 'rgba(148,163,184,0.15)',
...
function AgentTracePanel({ trace, stats }: { trace: any[]; stats: { time: number; calls: number; tokens: number } }) {
  const agentIcons: Record<string, any> = {
    'planner': <ApartmentOutlined style={{ color: '#1a5cff' }} />,
    'Master Planner': <ApartmentOutlined style={{ color: '#1a5cff' }} />,
    ...
    'replan': <ApartmentOutlined style={{ color: '#1a5cff' }} />,
    'attribution_agent': <CheckCircleOutlined style={{ color: '#06b6d4' }} />,
  };

  const agentColors: Record<string, string> = {
    'planner': '#1a5cff', 'Master Planner': '#1a5cff',
    ...
    'case_agent': '#8b5cf6', 'replan': '#1a5cff', 'attribution_agent': '#06b6d4',
  };
    ...
    <div>
      ...
      <Timeline
        ...
        items={trace.map((step: any) => ({
          color: agentColors[step.agent_name] || '#1a5cff',
          dot: agentIcons[step.agent_name] || <ClockCircleOutlined />,
          children: (
            <Card
              size="small"
              className="trace-card"
              style={{ borderLeftColor: agentColors[step.agent_name] || '#1a5cff' }}
